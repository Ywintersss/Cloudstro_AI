...
// Bedrock Agent IDs from console
...
export async function POST(req: Request) {
  ...
  try {
    ...
    // 2. Call Bedrock Agent
    const explanationText = await callBedrockAgent(
      "session-1", // could be user/session-specific if you want context memory
      `Explain this model output in plain English: ${rawPrediction}, 
      where the context of the output is the predicted engagement rate of a 
      social media platform based on past trends. 
      Don't show the entire raw value, round to 2 decimals. 
      Provide simple suggestions too.`
    );
...
const callBedrockAgent = async (sessionId: string, inputText: string) => {
  ...
  for await (const event of response?.completion!) {
    if (event.chunk?.bytes) {
      completion.push(new TextDecoder("utf-8").decode(event.chunk.bytes));
    }
  }
